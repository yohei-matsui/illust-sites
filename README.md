...
「参照画像で編集」タブで、画像1(体・構図を保つ側)と画像2(顔・人物の参照)を選び、英語の短い指示を書きます。既定の指示は「画像1の首から上を画像2の人物に置き換え、体・ポーズ・構図は画像1のまま、表情と目線は画像2」です。

精度を上げるコツ:

- 短い英語、肯定形。「絶対に」「一切」などの強調や否定形はモデルの挙動を変えません。
- 同一人物性は 1 回で決まらないので、シードを変えて数枚回し、当たりを選ぶ使い方が現実的です。
- CFG を 4 → 5〜6 に上げると指示に従いやすくなりますが、破綻も増えます。
- 体を 1 ピクセルも変えたくない場合は、生成に頼らずマスク付きインペイント + 同一人物アダプタ(PuLID など)を組む必要があります(このリポジトリには未実装)。
